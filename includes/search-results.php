...
class SearchResults {
  ...
  private function splitData($records) {
    foreach ($records as $record) {
      ...
      $record["pointer"] = '<a href="' . ROOT_FOLDER . 'viewer?type=transcript' . $link . '/"><img src="' . $image . $record["pointer"] . '" class="img-responsive" alt="' . $record["title"] . '"></a>';
  ...
  public function populateSpecimenData() {
    ...
    while ($statement->fetch()) {
      $data = array(
        "thumbnailurl"      => '<a href="' . ROOT_FOLDER . 'viewer?type=specimen&institute=Carolina&number=' . trim($otherCatalogNumbers) . '&search=' . $search . '/"><img src="' . trim($thumbnailurl) . '" class="img-responsive" alt="' . trim($scientificName) . '"></a>',
  ...
  public function scanImageEntries() {
    ...
    foreach (range(9089, 9149) as $pointer) {
      ...
      if (stripos($info["title"], $this->search) !== false) {
        array_push($array["images"], '<div class="col-xs-6"><a href="' . ROOT_FOLDER . 'viewer?type=transcript&institute=Carolina&number=' . $pointer . '&search=' . $this->search . '/"><img src="' . ROOT_FOLDER . 'img/gallery-small/' . $pointer . '.jpg" class="img-responsive"><p class="text-center">View Image</p></a></div>');
      } // if (stripos($info["title"], $this->search) !== false)
    } // foreach (range(9089, 9149) as $pointer)
